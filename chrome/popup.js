...
async function retrieveShortcut() {
    var storageItem = await chrome.storage.local.get("shortcut");
...
async function retrieveCustom() {
    var storageItem = await chrome.storage.local.get("custom");
...
runningSwitch.addEventListener("click", (ev) => {
    if (!runningSwitch.checked) {
        killCs();
        chrome.storage.local.get("disabled").then((it) => {
...
pdSwitch.addEventListener("click", (ev) => {
    if (!pdSwitch.checked) {
        hidePd();
        chrome.storage.local.get("noprompt").then((it) => {
...
document.getElementById("popup-header-digs").addEventListener("click", () => {
    window.open(chrome.runtime.getURL("digs.html"), "_blank").focus()
});
